...
if __name__ == "__main__":
    ...
    args = arg_parser()
    infile = "{}-{}.json".format(args.username, args.date)
    outfile_info = infile.replace(".json", ".csv")
    outfile_feat = infile.replace(".json", "_feat.csv")

    infile = os.path.join(JSON_DIR, infile)
    outfile_info = os.path.join(INFO_DIR, outfile_info)
    outfile_feat = os.path.join(FEAT_DIR, outfile_feat)
